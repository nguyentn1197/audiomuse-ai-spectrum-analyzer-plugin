...
- **Library scan, grouped by album** — same iteration as core analysis (`get_recent_albums(0)` → `get_tracks_from_album`), with per-album progress under Active Tasks.
...
- **Manual re-run** — a Re-analyze button on every track (runs on the `high` queue), plus three scan modes: *changed* (default), *verify* (re-hash everything), *force* (redo everything).
...
A 90 s segment from the middle of the track is loaded at native sample rate, STFT'd (n_fft 4096), and reduced to a robust per-frequency "max hold" profile (95th percentile over time). The cutoff is the highest frequency still within 40 dB of the 1–8 kHz reference level. A lossy encoder's low-pass leaves a near-vertical wall (high dB/kHz edge); a genuine master rolls off gradually. Verdict logic:

- cutoff ≈ Nyquist → `CLEAN`
- lossless container + low cutoff + sharp edge → `FAKE_SUSPECT` (estimated source class, e.g. "~128 kbps")
- lossless container + gradual rolloff → `LOWPASSED` (could be a genuine low-passed master — lower confidence)
...
These are heuristics — treat `FAKE_SUSPECT` as "look at the spectrogram," not a conviction.
...
- `plugins/SpectrumAnalyzer/__init__.py` — blueprint, pages (album overview, per-album detail), settings, migration, `register(ctx)`
- `plugins/SpectrumAnalyzer/jobs.py` — scan job, single-track re-run, fingerprints, hook, DB upsert
