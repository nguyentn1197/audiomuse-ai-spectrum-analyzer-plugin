...
- **Deep scan all non-CLEAN** — an album-page button that queues a deep scan for every track whose verdict isn't CLEAN, skipping verified and already-queued tracks. Runs on the default queue so parallel workers share the load.
