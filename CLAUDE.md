...
- **`dsp.py`** — Audio DSP (librosa + numpy) and analysis pipeline:
  ...
  - `_verdict()` decision logic: cutoff ≥ min(0.93 × Nyquist, 20.5 kHz) → CLEAN (genuine masters roll off at 20–21 kHz; near-Nyquist edges are meaningless — this deliberately lets ~320 kbps transcodes pass to avoid false accusations), unless the container is hi-res (> 48 kHz sample rate) and the cutoff aligns with a standard lower rate's Nyquist within 5% (`_resample_match`: 22.05/24/44.1/48 kHz) or sits below 23 kHz → UPSAMPLED (fake hi-res; confidence graded by edge sharpness + shelf silence, and boosted to 0.9 when `_alias_image_corr` finds the band above the cutoff mirroring the band below it — resampler aliasing images); lossless + sharp edge + digitally silent shelf (≤ −120 dB vs. reference) → FAKE_SUSPECT; sharp edge but audible noise above cutoff → LOWPASSED (encoder walls leave silence, masters leave dither); gradual rolloff → LOWPASSED; lossy with low cutoff relative to declared bitrate → TRANSCODED_LOSSY; otherwise CONSISTENT_LOSSY.
